...
class PrivateKey:
    def __init__(self, file_path, mod_value):
        ...
        if os.path.isfile(self.file_path):
            self.vectors = eval(Helpers.load_from_file(self.file_path))
        else:
            for _ in range(self.mod_value):
                self.vectors.append(Helpers.return_random_int(self.mod_value, True))
            Helpers.save_to_file(str(self.vectors), self.file_path)
...
PrivateKey("/run/media/neo/Neo/Projects/Programing/other/NeoCrypto/test_name_private", mod_value)
PublicKey("/run/media/neo/Neo/Projects/Programing/other/NeoCrypto/test_name_public", mod_value)
